...
| `src/bridge_for_agents/hook.py` | Codex stdio hook adapter for the HTTP bridge |
| `skills/notify-user/` | Agent skill: how to write a notification, and what never to put in one |
| `skills/bridge-ops/` | Agent skill: operating and diagnosing the bridge, and behaving well while gated by it |
...
| `examples/codex.hooks.json` | Codex command-hook configuration using the relay adapter |
